...
export const getPicture = function(userId, size){
  ...
  if(user!=null){
    if(user.profilePictureID!=null){
      ...
      if(picture!=null){
        url = Meteor.settings.public.CLOUDINARY_RES_URL + "/w_"+size+",h_"+size+",c_thumb,r_max" + "/v" + picture.media_version + "/" + Meteor.settings.public.LEVEL + "/" + user.profilePictureID;    
      }
    }
  }
  return url;
...
export const sendAlert = function(sender, receiver, message, entityId, entityType, alertType){
  Meteor.call(
    'sendAlert',
    sender,
    receiver,
